...
`openbimrs/<family>` repository. `ids/` and `icdd/` are extracted. Make family
changes in the child repository, pass its standalone gate, push the child
commit, and only then update the superproject pin. The root integration gate
must pass at the exact pin before it lands.
